...
def main():
    ...
    print("starting", start_epoch, current_iter)
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if dist.get_rank()==0 and current_iter % opt['logger']['print_freq'] == 0:
                ...
                # log_vars.update(model.get_current_log()[0])
                logs, log_d = model.get_current_log()
                print("Loss at iteration", current_iter, round(logs['l_pix'].item(),4), round(logs['GAN'].item(),4), round(logs['GAN_Feat'].item(), 4), round(logs['VGG'].item(),4), round(log_d['D_Fake'].item(),4), round(log_d['D_real'].item(),4))
                # logger.info("loger...%d"%current_iter)
                msg_logger(log_vars)
            ...
            if dist.get_rank() == 0 and current_iter % 800 == 1: 
                ...
                input_s = vis_results[0].permute(1,2,0).cpu().detach().numpy()
                gt_s = vis_results[1].permute(1,2,0).cpu().detach().numpy()
                out_s = vis_results[2].permute(1,2,0).cpu().detach().numpy()
