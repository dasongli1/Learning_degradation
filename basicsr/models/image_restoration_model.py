...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_loss.load_state_dict(torch.load("./checkpoints/net_E.pth"), strict=False)
        self.net_loss = self.model43_to_device(self.net_loss)
        ...
        if load_path is not None:
            self.load_network(self.net_g, load_path, False, param_key=self.opt['path'].get('param_key', 'params'))

        if self.is_train:
            self.init_training_settings()
    def model43_to_device(self, net):
    ...
    def optimize_parameters(self, current_iter):
        ...
        for kk in range(len(blur_list1)):
            ...
            l_blur += l_blur_ele
            # print("kk", kk, l_blur_ele)
        l_total +=  1.0 * l_blur
        loss_dict['l_blur'] = l_blur
        # perceptual loss
        ...
        self.log_g_dict = self.reduce_loss_dict(loss_dict)
        self.log_d_dict = None
    ...
    def get_latest_images(self):
        return [self.lq[0], self.gt[0], self.output[0]]
